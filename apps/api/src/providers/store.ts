...
export async function getProvider(id: string): Promise<ProviderRow | null> {
  const [row] = await sql<ProviderRow[]>`
    select id, kind, label, base_url, key_hint, key_set_by from provider where id = ${id}`;
  return row ?? null;
}
...
export async function setProviderKey(
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`update provider set key_hint = ${h}, key_set_by = ${setBy}, updated_at = now() where id = ${providerId}`;
  });
  return h;
}
...
export async function clearProviderKey(providerId: string): Promise<void> {
  await sql.begin(async (tx) => {
    ...
    await tx`update provider set key_hint = null, key_set_by = null, updated_at = now() where id = ${providerId}`;
  });
}
...
export async function resolveProvider(providerId: string): Promise<{
  ...
  keyEnv: string;
  key: string;
  /** The user who set the key: the owner every cost row is charged to. */
  ...
  return {
    kind: p.kind,
    baseUrl: p.base_url,
    keyEnv: KINDS[p.kind].keyEnv,
    key: k,
    keySetBy: p.key_set_by,
  };
}
