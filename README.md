...
Multiplayer GitHub issue triage dashboard. Issues sync from GitHub into Postgres, Rocicorp Zero
replicates them to every browser, a Hono API classifies them with TypeSafe System One, and users
correct the model; corrections persist and feed the next recalculation.
...
Open http://localhost:5173, sign in with any name, paste `owner/name`, press Sync.
...
- **Sync** (`POST /api/sync {owner, name, paused?}`): Octokit paginates `issues.listForRepo`
  sorted by `updated` ascending with `since = repo.sync_cursor`; pull requests are skipped; each
  page is one transaction so the table fills in live; the cursor advances per page.
