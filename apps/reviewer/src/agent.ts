...
import { completeWithPi, modelFor, models, type Completion, type ProviderSpec } from "./pi.ts";
 ...
 * are executed here, appended to the context, and the model is asked again, up to a cap.
 */
...
export async function runTool(
  ...
  if (!snap) return { text: "no repository snapshot is available for this review", isError: true };
  ...
  switch (name) {
    ...
    case "rank_files": {
      ...
      return {
        text: data.ranked.map((r) => `${r.relevance.toFixed(2)}  ${r.path}`).join("\n") || "(none)",
        isError: false,
      };
    }
    default:
...
/** Ask with tools until the model answers in text; without a snapshot this is one completion. */
export async function runAgent(
  ...
  if (!host.snapshot) return { ...(await completeWithPi(spec, modelId, prompt)), toolCalls: 0 };
  const m = models();
  ...
  for (let turn = 0; turn < MAX_TURNS; turn += 1) {
    const r = await m.completeSimple(
      model,
